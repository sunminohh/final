...
// 댓글 관련 코드
		...
		let commentNo = $(this).closest(".child-comment-box").attr("data-cancel-comment");
		...
		$(".great-box").on('click','#btn-comment' ,function() {
			...
			let newContent = $that.closest(".child-comment-box").find("#new-content-div textarea").val();
		    ...
		    if(newContent === "") { // 댓글 내용이 비어있을 때
			    ...
			    $.ajax({
			        ...
			        success: function(comment) {
						...
						$that.closest('.great-box').find('.re-comment-here').append(content);
						
						 // 답글 작성 폼을 제거하고 답글 쓰기 버튼으로 변경
						$that.closest('.great-box').find('.new-register-box ').remove();
            		    $that.closest('.great-box').find("#btn-a-re-reply-" + result).attr('id', 'btn-a-reply-' + result).text('답글쓰기');
...
// 쌤한테 물어보기 - ajax 삭제는 되는데 사라지지 않음
// ajax 상태에서 답글쓰기 되지만 표시한됨
// 취소로 안바뀜 - 304라인
